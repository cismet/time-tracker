...
//        }
     ...
     * @param   work  speichert die berechnete Arbeitszeit. Wenn autoPause des work-Objektes gesetzt ist, dann wird 1
     *                Stunde Pause abgezogen, falls keine Pause manuell eingetragen wurde und Arbeitszeit >
     *                TimeTrackerConstants.HOURS_WITHOUT_AUTO_PAUSE.
     *
     ...
     */
    public void getHoursOfWork(final int u_id, final GregorianCalendar day, final HoursOfWork work) throws SQLException,
        QueryException {
        getHoursOfWork(u_id, day, work, null, null, true);
    }

    /**
     * liefert die Tagesarbeitszeit des uebergebenen Benutzers am uebergebenen Tag (todo: Methode ueberarbeiten ist zu
     * gross)
     *
     * @param   u_id             Benutzer-ID
     * @param   time             day Tagt, fuer den die Arbeitszeit berechnet werden soll
     * @param   work             speichert die berechnete Arbeitszeit. Wenn autoPause des work-Objektes gesetzt ist,
     *                           dann wird 1 Stunde Pause abgezogen, falls keine Pause manuell eingetragen wurde und
     *                           Arbeitszeit > TimeTrackerConstants.HOURS_WITHOUT_AUTO_PAUSE.
     ...
     * @param   withCorrections  gibt an, ob Korrekturen beruecksichtigt werden sollen. Wenn false, dann werden nur
     *                           Korrekturen ohne Projekt beruecksichtigt
     *
     * @throws  SQLException    wird bei Datenbankfehler geworfen
     * @throws  QueryException  wird geworfen, wenn die, zur Kommen-Buchung gehoerende, Gehen-Buchung fehlt
     */
    public void getHoursOfWork(final int u_id,
            final GregorianCalendar time,
            final HoursOfWork work,
            final GregorianCalendar fromTime,
            final GregorianCalendar toTime,
            final boolean withCorrections) throws SQLException, QueryException {
        work.setNeedAutoPause(false);
        final GregorianCalendar day = (time == null) ? new GregorianCalendar() : time;
        final boolean isToday = TimeTrackerFunctions.getDateString(day)
                    .equals(TimeTrackerFunctions.getDateString(new GregorianCalendar()));
        work.setHours(0.0);
        final SimpleDateFormat formaterDB = new SimpleDateFormat("yyyy-MM-d");
        // System.out.println(TimeTrackerConstants.dateFormater.format(day.getTime()));
        final boolean netHoursOfWork = db.isUserInNetMode(u_id, day);
        final TimesheetSet set = db.getTimeOfWork(u_id, day);
...
//                            work.setHours(work.getHours() + entry.getDuration_in_hours());
                    ...
                    if ((fromTime == null) && (toTime == null)) {
                        ...
                        } else {                                                              // Moegliches Ende einer
                                                                                              // Pause
                            if (startPause != 0) {
                                long endPause = entry.getTime().getTimeInMillis();

                                if ((toTime != null)
                                            && (toTime.getTimeInMillis() < entry.getTime().getTimeInMillis())) {
            ...
            final boolean hasAutoPause = db.hasAutopause(u_id);
     ...
     */
    public double getHoursOfWorkInWeek(final int u_id) throws SQLException {
     ...
     */
    public double getHoursOfWorkInMonth(final int u_id) throws SQLException, QueryException {
        final GregorianCalendar now = new GregorianCalendar();
     ...
     * @throws  QueryException  DOCUMENT ME!
     */
    public double getHoursOfWorkInMonth(final int u_id, final int month, final int year) throws SQLException,
        QueryException {
        HoursOfWork how = new HoursOfWork();
        double hoursInMonth = 0.0;
        boolean firstDay = true;
        final GregorianCalendar currentDay;
     ...
     * @throws  SQLException  DOCUMENT ME!
     */
    public String getHolidayTable(final int u_id, final int year) throws SQLException {
        ...
        final ResultSet holiday = db.getHolidaysForYear(u_id, toTime);
        final String table = getTableOfYear(u_id, holiday, null, toTime);
        holiday.close();
     ...
     * @throws  QueryException  DOCUMENT ME!
     */
    public String getMonthTable(final int u_id, final int year) throws SQLException, QueryException {
        ...
        final StringBuffer table = new StringBuffer("<table border=\"5\" cellspacing=\"5\" width=\"100%\">");
        ...
        while (time.get(GregorianCalendar.YEAR) == year) {
            table.append("<tr><td>" + months[time.get(GregorianCalendar.MONTH)]
                        + "</td><td>"
                        + TimeTrackerFunctions.convert2Time(
                            getHoursOfWorkInMonth(u_id, time.get(GregorianCalendar.MONTH), year),
                            false)
                        + "</td></tr>");
     ...
     * @throws  SQLException  DOCUMENT ME!
     */
    public String getYearTable(final int u_id, final int year) throws SQLException {
        GregorianCalendar toTime = new GregorianCalendar();
        ...
        final ResultSet holiday = db.getHolidaysForYear(u_id, toTime);
        final ResultSet illness = db.getIllnessForYear(u_id, toTime);

        final String table = getTableOfYear(u_id, holiday, illness, toTime);
        holiday.close();
        illness.close();
    ...
    /**
     * liefert eine Tabelle mit einem Jahresueberblick Wenn illness == null, dann werden nur die Urlaubstage eingetragen
     * Sonst werden Fehltage wegen Krankheit, Urlaub und die Tagesarbeitszeit.
     *
     * @param   u_id     DOCUMENT ME!
     * @param   holiday  DOCUMENT ME!
     * @param   illness  DOCUMENT ME!
     ...
     * @throws  SQLException  DOCUMENT ME!
     */
    private String getTableOfYear(final int u_id,
            final ResultSet holiday,
            final ResultSet illness,
            final GregorianCalendar toTime) throws SQLException {
        final StringBuffer table = new StringBuffer();
        final StringBuffer row = new StringBuffer();
        final StringBuffer headRow = new StringBuffer("<table border=\"5\" cellspacing=\"5\" width=\"100%\">");
        final String[] dayOfWeek = { "Mo", "Di", "Mi", "Do", "FR", "Sa", "So" };
        final int year = (toTime).get(GregorianCalendar.YEAR);
        String value;
        ...
        final GregorianCalendar now = toTime;
        double week = 0.0;
        double sum = 0.0;
        final Holidays holidays = new Holidays(this.db);
        final StringBuffer holidaySymbol = new StringBuffer();
        double holidayTime;
        ...
        final GregorianCalendar date = new GregorianCalendar(gc.get(GregorianCalendar.YEAR),
        ...
        while (((illness != null) && (gc.before(now) || TimeTrackerFunctions.isSameDate(gc, now)))
                    || ((illness == null) && (gc.get(GregorianCalendar.YEAR) == year))) {
            ...
            if (gc.get(GregorianCalendar.DAY_OF_WEEK) == GregorianCalendar.MONDAY) {     // neue Woche ==> KW eintragen
                if ((illness != null) && (gc.get(GregorianCalendar.DAY_OF_YEAR) != 1)) { // Ausgabe Wochenarbeitszeit
                    final GregorianCalendar lastSunday = (GregorianCalendar)gc.clone();
                    lastSunday.add(GregorianCalendar.DATE, -1);
                    final double saldo = week - getTargetHoursPerWeek(u_id, lastSunday, null, true);
            ...
            if ((nextHoliday != null)
                        && (nextHoliday.get(GregorianCalendar.DAY_OF_YEAR) == gc.get(GregorianCalendar.DAY_OF_YEAR))) {
                // Urlaub
                final StringBuffer valueTmp = new StringBuffer();
                week += addAbsentDay(u_id, gc, valueTmp, holiday, nextHoliday, false, holidayTime);
                value = holidaySymbol.toString() + valueTmp + "U";

                if ((illness != null) && (nextIllness != null)
                            && (nextIllness.get(GregorianCalendar.DAY_OF_YEAR)
                    ...
                    {
                        final HoursOfWork work = new HoursOfWork(false);
            ...
            } else if ((illness != null) && (nextIllness != null)
                        && (nextIllness.get(GregorianCalendar.DAY_OF_YEAR) == gc.get(GregorianCalendar.DAY_OF_YEAR))) {
                // Krank
                final StringBuffer valueTmp = new StringBuffer();
                ...
                try {
                    final HoursOfWork work = new HoursOfWork(true);
                    getHoursOfWork(u_id, gc, work);
                    week += work.getHours();
    ...
    /**
     * Wenn der uebergebene Tag ein Feiertag ist, dannn werden die Stunden, die dem uebergebenen User aufgrund des
     * Feiertags zustehen zurueckgeliefert. Sonst Rueckgabe 0.0
     *
     * @param   u_id           ID des Benutzers
     * @param   day            Tag, der auf Feiertag geprueft wird
     * @param   holidaySymbol  darin wird der Grund der zurueckgelieferten Stunden gespeichert. (F fuer Feiertag, H fuer
     *                         halber Feiertag oder "" fuer Arbeitstag) Diese Variable wird von der Methode veraendert.
     ...
     */
    private double getHolidayTime(final int u_id,
            final GregorianCalendar day,
            StringBuffer holidaySymbol,
            final Holidays holidays) throws SQLException {
        ...
        if (holidays.isHoliday(day) != Holidays.WORKDAY) {
            if (holidays.isHoliday(day) == Holidays.HALF_HOLIDAY) {
                ...
                if ((day.get(GregorianCalendar.DAY_OF_WEEK) != GregorianCalendar.SATURDAY)
                            && (day.get(GregorianCalendar.DAY_OF_WEEK) != GregorianCalendar.SUNDAY)) {
                    // zusaetzliche Zeit wird nur fuer Feiertage angerechnet, die nicht auf einem Wochenende liegen
                    holidayTime = getTargetHoursPerWeek(u_id, (GregorianCalendar)day.clone(), null, false)
     ...
     * @param  saldo  DOCUMENT ME!
     * @param  day    DOCUMENT ME!
     * @param  u_id   DOCUMENT ME!
     */
    private void appendWeekFazit(final StringBuffer table,
        ...
        row.append("<td style=\"color:" + ((saldo < 0) ? "red" : "green") + "\">"
                    + TimeTrackerFunctions.convert2Time(saldo, true) + "</td></tr>\n");
        table.insert(0, row);
        row.delete(0, row.length());
    }

    /**
     * liefert die Beschreibung eines Tages mit abwesenden Stunden eingesetzt bei der Jahresstatistik gesamt.
     *
     * @param   u_id           DOCUMENT ME!
     * @param   currentDate    DOCUMENT ME!
     * @param   value          DOCUMENT ME!
     * @param   absentDays     DOCUMENT ME!
     * @param   nextAbsentDay  DOCUMENT ME!
     * @param   isIllness      DOCUMENT ME!
     * @param   holidayTime    DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     *
     * @throws  SQLException  DOCUMENT ME!
     */
    private double addAbsentDay(final int u_id,
            final GregorianCalendar currentDate,
            final StringBuffer value,
            final ResultSet absentDays,
            GregorianCalendar nextAbsentDay,
            final boolean isIllness,
            final double holidayTime) throws SQLException {
        double time = 0.0;
        final double targetHoursPerDay = getTargetHoursPerWeek(u_id, currentDate, null, false)
                    / TimeTrackerConstants.WORK_DAYS_PER_WEEK;
        ...
        if (absentDays.getMetaData().getColumnCount() == 2) { // trifft wahrscheinlich immer zu
            ...
            try {
                ...
                if (work.getHours() != 0.0) {
                    if (isIllness && ((time + work.getHours()) > targetHoursPerDay)) {    // Abwesenheit durch Krankheit
                                                                                          // wird gekuerzt, so dass
                                                                                          // keine Ueberstunden durch
                                                                                          // Krankheit entstehen
                        time = targetHoursPerDay - work.getHours();
                        if (time < 0) {
                            time = 0;
                        }
                        value.append(TimeTrackerFunctions.convert2Time(work.getHours(), false) + "/"
                                    + TimeTrackerFunctions.convert2Time(time, false));
                    } else {
                        value.append(TimeTrackerFunctions.convert2Time(work.getHours(), false) + "/"
                                    + TimeTrackerFunctions.convert2Time(time, false));
                    }
     ...
     * @param   validTill        darin wird der Zeitpunkt gespeichert, zu dem der zur Zeit day laufende Vertrag
     *                           auslaeuft Wenn dieser Zeitpunkt nicht benoetigt wird, dann kann null uebergeben werden.
     *                           Wenn Auslauf-Datum offen, dann validTill.get(GregorianCalendar.YEAR) ==
     *                           getMaximum(GregorianCalendar.YEAR).
     * @param   averageOverWeek  Wenn averageOverWeek == true, dann wird der Durchschnittswert der angegebenen Woche
     *                           ermittlet. Tage aus dem alten Jahr zaehlen dabei nicht.
     *
     * @return  die Sollstunden der angegebenen Woche
     */
    public double getTargetHoursPerWeek(final int u_id,
            final GregorianCalendar day,
            final GregorianCalendar validTill,
            final boolean averageOverWeek) {
        ...
        try {
            final ContractInfos contract = db.getHoursOfWork(u_id, tmpDay);
            double whow;
    ...
    /**
     * liefert die durchschnittliche Sollstunden der angegebenen Woche. D.h. Vertragsaenderungen waehrend der Woche
     * werden beruecksichtigt. Die Woche beginnt immer mit dem letzten Montag vor dem uebergebenen Datum. Sonst wie
     * getTargetHoursPerWeek
     *
     * @param   u_id       DOCUMENT ME!
     * @param   day        DOCUMENT ME!
     * @param   validTill  DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     */
    private double getTargetHoursAverage(final int u_id,
            final GregorianCalendar day,
            final GregorianCalendar validTill) {
        GregorianCalendar tmpDay = (GregorianCalendar)day.clone();
        ...
        final int year = tmpDay.get(GregorianCalendar.YEAR);

        while ((tmpDay.get(GregorianCalendar.DAY_OF_WEEK) != GregorianCalendar.MONDAY)
                    && (tmpDay.get(GregorianCalendar.YEAR) == year)) {
     ...
     * @throws  SQLException  DOCUMENT ME!
     */
    public double getIllnessDays(final int u_id) throws SQLException {
        final double timeOfWork = getTargetHoursPerWeek(u_id, new GregorianCalendar(), null, false)
                    / TimeTrackerConstants.WORK_DAYS_PER_WEEK;
    ...
    /**
     * fuellt das uebergebene Holiday-Objekt. Wenn year == null, dann gilt das aktuelle Jahr.
     *
     ...
     * @throws  SQLException  DOCUMENT ME!
     */
    public void getHoliday(final int u_id, GregorianCalendar year, final HolidayStruct holidays) throws SQLException {
        if (year == null) {
        ...
        final Vector<TimeDurationPair> usedHolidays = db.getUsedHolidaysForYear(u_id, year);

        if (usedHolidays != null) {
            for (final TimeDurationPair timeDuration : usedHolidays) {
                double holidaysPerDay = 1.0;
                if (timeDuration.getDuration() != 0) {
                    final double daysOfWorkPerWeek = (getTargetHoursPerWeek(u_id, timeDuration.getTime(), null, false)
                                    / TimeTrackerConstants.WORK_DAYS_PER_WEEK);
        ...
        if (yearCopy.get(GregorianCalendar.YEAR) > getYearOfFirstContract(u_id)) {
            final HolidayStruct holidaysLastYear = new HolidayStruct();
            final GregorianCalendar lastYear = (GregorianCalendar)yearCopy.clone();
        ...
        final Vector<TimeDurationPair> holidayCorrections = db.getHolidayCorrectionsForYear(u_id, year);

        double manualAmount = 0.0;
        if (holidayCorrections != null) {
            for (final TimeDurationPair timeDuration : holidayCorrections) {
     ...
     * @throws  SQLException  DOCUMENT ME!
     */
    private int getYearOfFirstContract(final int u_id) throws SQLException {
        final GregorianCalendar tmp;
     ...
     * @throws  SQLException  DOCUMENT ME!
     */
    public String getCurrentAccountBalance(final int u_id) throws SQLException {
        ...
        final String holidayQuery = db.getHolidayQueryString(u_id, lastReset);
        final ResultSet holiday = db.execute(holidayQuery);

        final String illnessQuery = db.getIllnessQueryString(u_id, lastReset);
        final ResultSet illness = db.execute(illnessQuery);
        ...
        final GregorianCalendar now = new GregorianCalendar();
        final GregorianCalendar tmpTime = (GregorianCalendar)lastReset.clone();
        GregorianCalendar contractValidTill = null;

        final HoursOfWork time = new HoursOfWork();
        double dailyTimeOfWork = 0.0;

        while (!TimeTrackerFunctions.getDateString(tmpTime).equals(TimeTrackerFunctions.getDateString(now))
                    && tmpTime.before(now)) {
            ...
            if (((contractValidTill != null) && contractValidTill.before(tmpTime)) || (contractValidTill == null)) { // neue Wochenarbeitszeit anfordern
                ...
                dailyTimeOfWork = (getTargetHoursPerWeek(u_id, tmpTime, contractValidTill, false)
                                / TimeTrackerConstants.WORK_DAYS_PER_WEEK);
            }
            ...
            if ((tmpTime.get(GregorianCalendar.DAY_OF_WEEK) != GregorianCalendar.SATURDAY)
                ...
                if ((nextIllness != null)
                            && TimeTrackerFunctions.getDateString(nextIllness).equals(
                                TimeTrackerFunctions.getDateString(tmpTime))) {    // wenn ganzer oder halber Fehltag
                                                                                   // wegen Krankheit
                    double holidayDuration = 0.0;

                    if ((nextHoliday != null)
                                && TimeTrackerFunctions.getDateString(nextHoliday).equals(
                                    TimeTrackerFunctions.getDateString(tmpTime))) {
                    ...
                    final double illnessDuration = (illness.getDouble(2) == 0.0) ? dailyTimeOfWork
                                                                                 : illness.getDouble(2);
                    if ((illnessDuration + time.getHours() + holidayTime + holidayDuration) < dailyTimeOfWork) {
                        accountBalance += (time.getHours() + illnessDuration + holidayDuration + holidayTime)
                                    - dailyTimeOfWork;
                    }
                    if (illness.next()) {
                        nextIllness.setTimeInMillis(illness.getTimestamp(1).getTime());
                        ...
                        nextIllness = null;
                            ...
                            && TimeTrackerFunctions.getDateString(nextHoliday).equals(
                                TimeTrackerFunctions.getDateString(tmpTime))) { // wenn ganzer oder halber Urlaubstag
                    final double holidayDuration = (holiday.getDouble(2) == 0.0) ? dailyTimeOfWork
                                                                                 : holiday.getDouble(2);
     ...
     * @param   onlyMonth  DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     *
     * @throws  SQLException  DOCUMENT ME!
     */
    public Hashtable getProjectTimes(final int u_id, final GregorianCalendar date, final boolean onlyMonth)
            throws SQLException {
        final Hashtable projectTimes = new Hashtable();
        GregorianCalendar projectStart = new GregorianCalendar(date.get(GregorianCalendar.YEAR),
        ...
        final GregorianCalendar firstProjectDay = new GregorianCalendar();
        String timeInterval;
        final GregorianCalendar endOfTimeInterval = new GregorianCalendar();
        TitleTimePair projectStartStruct = null;

        // endOfTimeInterval auf das Ende des gegebenen Zeitintervalls setzen
        if (onlyMonth) {
            endOfTimeInterval.set(date.get(GregorianCalendar.YEAR),
                date.get(GregorianCalendar.MONTH),
        ...
        final Vector<TitleTimePair> projectStartVector = db.getProjectComes(u_id, date, timeInterval);
        final Iterator<TitleTimePair> projectStartIt = projectStartVector.iterator();

        final Vector<ProjectInfos> projectSubsequents = db.getProjectSubsequents(u_id, date, timeInterval);
        final Iterator<ProjectInfos> projectSubIt = projectSubsequents.iterator();
        ...
        if (!projectStartIt.hasNext()) {
            ...
            while (nextSubsequentProject != null) {
                if ((nextSubsequentProject.getAction() != null) && (nextSubsequentProject.getTitle() != null)) {
                    insertIntoHashtable(
                        projectTimes,
                        nextSubsequentProject.getTitle(),
                        nextSubsequentProject.getDuration());
                }
        ...
        while ((nextSubsequentProject != null)
                    && (nextSubsequentProject.getTime().getTimeInMillis() < projectStart.getTimeInMillis())) {
            if ((nextSubsequentProject.getAction() != null) && (nextSubsequentProject.getTitle() != null)) {
                insertIntoHashtable(
                    projectTimes,
                    nextSubsequentProject.getTitle(),
        ...
        while ((projectStart != null)
                    && !TimeTrackerFunctions.getDateString(projectStart).equals(
            ...
            if (projectStartStruct != null) {
                ...
                final GregorianCalendar tmpTime = (GregorianCalendar)projectStart.clone();
                final HoursOfWork time = new HoursOfWork(true);
                final String lastProjectTitle = projectStartStruct.getTitle();
                ...
                while (TimeTrackerFunctions.isDateLess(tmpTime, projectEnd)
                    ...
                    try {
                        if (
                            !TimeTrackerFunctions.getDateString(tmpTime).equals(
                                        TimeTrackerFunctions.getDateString(firstProjectDay))) {
                            final GregorianCalendar tmpStartTime = new GregorianCalendar(tmpTime.get(
                            ...
                            time.setAutoPause(true);
                            // Startzeit wird angegeben, damit die Pausen, ausser die AutoPause, nicht beruecksichtigt
                            // werden
                ...
                try {
                    if (TimeTrackerFunctions.getDateString(projectStart).equals(
                                    TimeTrackerFunctions.getDateString(projectEnd))) {
                ...
                if (nextSubsequentProject != null) {
                    ...
                    while ((nextSubsequentProject != null) && !projectToLate) {
                        if ((nextSubsequentProject.getAction() != null)
                                    && nextSubsequentProject.getAction().equals("CORRECTION")) {
                        ...
                        if (projectSubIt.hasNext()) {
                            ...
                            if ((tmp.getTimeInMillis() > projectEnd.getTimeInMillis())
                                        || (tmp.getTimeInMillis() > endOfTimeInterval.getTimeInMillis())) {
                                projectToLate = true;
                            }
                        } else {
                            projectToLate = true;
     ...
     */
    private void insertIntoHashtable(final Hashtable ht, final String name, final double add) {
        final Double tmp;
