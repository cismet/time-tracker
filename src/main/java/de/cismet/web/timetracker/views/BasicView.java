        ...
        try {
            if (item.getAction().equals(TimeTrackerConstants.WORKDAY_STRING)) {
                ...
                String sqlString = "INSERT INTO tt_timesheet" +
                        ...
                        " VALUES ('" + formaterDB.format(item.getStart().getTime()) + "', " + TimeTrackerConstants.COME +
                        ", null, true, " + u_id + ", null, null)";
                db.executeUpdate(sqlString);
                //gehen-Buchung
                sqlString = "INSERT INTO tt_timesheet" +
                        " (time, action, annotation, manual, u_id, project_id, duration_in_hours)" +
                        " VALUES ('" + formaterDB.format(item.getEnd().getTime()) + "', " + TimeTrackerConstants.GO +
                        ", null, true, " + u_id + ", null, null)";
                ...
                long duration = (item.getEnd().getTimeInMillis() - item.getStart().getTimeInMillis());
            ...
            ResultSet oidSet = db.execute(sqlString);
            if (oidSet.next()) {
    ...
    public boolean saveModifiedItem(CalendarItem item, int u_id) {
        ...
        try {
            ...
            if (original.getStart().get(GregorianCalendar.DATE) != item.getStart().get(GregorianCalendar.DATE)) {
                ...
                long minutes = Math.abs(original.getStart().getTimeInMillis() / (1000 * 60) - item.getStart().getTimeInMillis() / (1000 * 60));
                char sign = (original.getStart().before(item.getStart()) ? '+' : '-');

                sqlString = "UPDATE tt_timesheet SET time = time " + sign + " INTERVAL '" + minutes + " minutes' " +
                        ", manual=true WHERE u_id=" + u_id + " AND " +
                        "date_trunc('day', time) = date_trunc('day', timestamp '" + formaterDB.format(original.getStart().getTime()) + "') AND " +
                        "(action = " + TimeTrackerConstants.COME + " OR action = " + TimeTrackerConstants.GO + " OR action = " + TimeTrackerConstants.HOLIDAY_HOURS +
                        " OR action = " + TimeTrackerConstants.ILLNESS_HOURS + " OR action = " + TimeTrackerConstants.PROJECT_SUBSEQUENT +
                        " OR action = " + TimeTrackerConstants.PROJECT_COME + " OR action = " + TimeTrackerConstants.CORRECTION + ")";
//				System.out.println(sqlString);
                db.executeUpdate(sqlString);
            } else {
                if (!TimeTrackerConstants.formater.format(original.getStart().getTime()).equals(TimeTrackerConstants.formater.format(item.getStart().getTime()))) {
                    //Startzeit hat sich geaendert, der Tag aber nicht
                    sqlString = "UPDATE tt_timesheet SET time = '" + formaterDB.format(item.getStart().getTime()) + "', manual=true WHERE oid=" + item.getId();
                    db.executeUpdate(sqlString);
                    ...
                    sqlString = "DELETE FROM tt_timesheet WHERE u_id=" + u_id + " AND " +
                            "date_trunc('day', time) = date_trunc('day', timestamp '" + formaterDB.format(item.getStart().getTime()) + "') AND " +
...
//					System.out.println(sqlString);
                ...
                if (!TimeTrackerConstants.formater.format(original.getEnd().getTime()).equals(TimeTrackerConstants.formater.format(item.getEnd().getTime()))) {
                    ...
                    sqlString = "SELECT oid FROM tt_timesheet " +
                            ...
                            "AND action = " + TimeTrackerConstants.GO + " AND u_id = " + u_id + " ORDER BY time DESC LIMIT 1";

                    ResultSet lastGo = db.execute(sqlString);
                    if (lastGo.next()) {
                        sqlString = "UPDATE tt_timesheet SET time = '" + formaterDB.format(item.getEnd().getTime()) + "', manual=true WHERE oid=" + lastGo.getString(1);
                        db.executeUpdate(sqlString);
                    } else {
                        return false;
                    }
    ...
    public Vector getItemsOfDay(GregorianCalendar day, int u_id) throws SQLException {
        Vector calendarItems = new Vector();
        String sqlQuery = getQueryString(day, u_id);
        ...
        if (rSet != null) {
            ...
            while (rSet.next()) {
                ...
                if (rSet.getInt(2) == TimeTrackerConstants.COME && firstCome) {					//Arbeitsstart ermitteln
                    ...
                    if (startPause != 0) {
                        long endPause = rSet.getTimestamp(9).getTime();
                    ...
                    entryDesc += "<br />" + rSet.getString(7) + ": " + TimeTrackerFunctions.convert2Time(rSet.getDouble(8), false);
                } else if (rSet.getInt(2) == TimeTrackerConstants.GO) {							//Arbeitszeit wird durch ein GO unterbrochen
                ...
                } else if (rSet.getInt(2) == TimeTrackerConstants.HOLIDAY_HOURS || rSet.getInt(2) == TimeTrackerConstants.ILLNESS_HOURS) {	//Urlaub oder Krank

                    String eventName = (rSet.getInt(2) == TimeTrackerConstants.HOLIDAY_HOURS ? TimeTrackerConstants.HOLIDAY_STRING : TimeTrackerConstants.ILLNESS_STRING);
                    if (start == null) {											//zu Beginn des Tages Urlaub/Krank
                        ...
                        if (!hasRelevantEntry(rSet)) {							//ganzer Tag Urlaub/Krank
                            ...
                            double dailyHoursOfWork = q.getTargetHoursPerWeek(u_id, day, null, false) / TimeTrackerConstants.WORK_DAYS_PER_WEEK;
                            double duration = (rSet.getDouble(8) == 0 ? dailyHoursOfWork : rSet.getDouble(8));
                        ...
                        entryDesc += "<br />" + eventName + ": " + TimeTrackerFunctions.convert2Time(rSet.getDouble(8), false);
                        hoursOfWork -= rSet.getDouble(8);
                    }
                    ...
                    if (start != null || hasRelevantEntry(rSet)) {
                        entryDesc += "<br />zus&auml;tzlich " + TimeTrackerFunctions.convert2Time(rSet.getDouble(8), false) + "h Korrektur";
                ...
                } else if (rSet.getInt(2) == TimeTrackerConstants.COME) {					//folgt nicht unmittelbar auf ein go und ist nicht die erste Buchung
                    if (action == null) {
                        action = TimeTrackerConstants.WORKDAY_STRING;
                    }
                }
            ...
            if (start != null && end != null && action != null) {
                if (action.equals(TimeTrackerConstants.HOLIDAY_STRING) || action.equals(TimeTrackerConstants.ILLNESS_STRING) || action.equals(TimeTrackerConstants.CORRECTION_STRING)) {
                    entryDesc = "";
                }
                CalendarItem item = new CalendarItem(action, entryDesc, end, id, null, start);
                calendarItems.add(item);
            } else if (start != null && end != null && action == null) {
                System.out.println("action == null, id == " + id + " am " + TimeTrackerConstants.formater.format(day.getTime()));
            } else {
                //System.out.println("Fuer den " + TimeTrackerConstants.formater.format(day.getTime()) + " sind keine (zumindest keine fehlerfreien) Daten verfuegbar.");
            }
        } else {
            System.out.println(db.getErrorMessage());
        }
//		System.out.println(xmlResult.toString());
     ...
     */
    private String getQueryString(GregorianCalendar day, int u_id) {
     ...
     */
    private boolean hasRelevantEntry(ResultSet rSet) throws SQLException {
     ...
     */
    private boolean changeAction(CalendarItem original, CalendarItem newItem, int u_id) {
        try {
            int action;
            ...
            if (original.getAction().equals(TimeTrackerConstants.WORKDAY_STRING)) {
                ...
                String sqlString = "UPDATE tt_timesheet SET action = " + action + ", duration_in_hours = " +
                        TimeTrackerFunctions.calcDifferenceInHours(newItem.getEnd(), newItem.getStart()) + " WHERE " +
                        " oid = " + original.getId();
                ...
                sqlString = "DELETE FROM tt_timesheet WHERE u_id = " + u_id +
                        " AND date_trunc('day', time) = date_trunc('day', timestamp '" + formaterDB.format(newItem.getStart().getTime()) + "') " +
                        " AND ( (action = " + TimeTrackerConstants.COME + " OR action = " + TimeTrackerConstants.GO + ") )";
                /*
                 * PROJECT COMEs und PROJECT SUBSEQUENTs in Verbindung mit Pause wird zwar nicht mehr benoetigt,
                 * eine Loeschung macht aber eine Rueckkonvertierung unmoeglich. Eventuell folgende anfuegen:
                 * "OR (action = " + PROJECT_COME + " AND project_id = " + PROJECT_PAUSE + ") " +
                 * "OR (action = " + PROJECT_SUBSEQUENT + " AND project_id = " + PROJECT_PAUSE + ") )";
                 */
                db.executeUpdate(sqlString);
                ...
                if (newItem.getAction().equals(TimeTrackerConstants.WORKDAY_STRING)) {
                    ...
                    sqlString = "INSERT INTO tt_timesheet (time, action, u_id) VALUES('" +
                            formaterDB.format(newItem.getEnd().getTime()) + "', " + TimeTrackerConstants.GO + ", " + u_id + ")";

                    db.executeUpdate(sqlString);

                } else {
                    // von Urlaub/Krankheit oder Korrektur zu Urlaub/Krankheit oder Korrektur
                    String sqlString = "UPDATE tt_timesheet SET action = " + action + ", project_id = null WHERE " +
                            " oid = " + original.getId();
                    db.executeUpdate(sqlString);
                }
            }
    ...
    public static void main(String[] args) {
        ...
        GregorianCalendar cal = new GregorianCalendar(2006,7,31);
        //		while (cal.get(GregorianCalendar.MONTH) < 8){
        try{
        System.out.println(TimeTrackerConstants.formater.format(cal.getTime()));
        bi.getItemsOfDay(cal, 1);
